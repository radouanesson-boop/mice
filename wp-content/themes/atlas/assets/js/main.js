...
(function () {
	...
	const config = window.dmfConfig || { breakpointDesktop: 1024, i18n: {} };

	/* -----------------------------------------------------------------------
	 * Sticky header state — adds .is-stuck for the shadow.
	 * -------------------------------------------------------------------- */
	function initStickyHeader() {
		const header = document.querySelector('[data-dmf-header]');
		if (!header) return;
	...
	function initMobileNav() {
		const drawer = document.querySelector('[data-dmf-mobile-nav]');
		const toggle = document.querySelector('[data-dmf-nav-toggle]');
		if (!drawer || !toggle) return;

		const panel = drawer.querySelector('.dmf-mobile-nav__panel');
		let lastFocus = null;
		...
		drawer.addEventListener('click', (event) => {
			if (event.target.closest('[data-dmf-nav-close]')) setOpen(false);
		});
		...
		drawer.querySelectorAll('.dmf-mobile-nav__menu .menu-item-has-children > a').forEach((link) => {
	...
	function initDesktopNav() {
		document.querySelectorAll('.dmf-nav__item--parent > a, .dmf-nav__item--mega > a').forEach((link) => {
			link.addEventListener('click', (event) => {
		...
		function closeAllPanels() {
			document.querySelectorAll('.dmf-nav__item.is-open').forEach((li) => {
		...
		document.addEventListener('click', (event) => {
			if (!event.target.closest('.dmf-nav')) closeAllPanels();
		});
	...
	function initCounters() {
		const counters = document.querySelectorAll('[data-dmf-counter]');
		...
		const observer = new IntersectionObserver((entries) => {
			entries.forEach((entry) => {
				...
				const target = parseFloat(entry.target.dataset.dmfCounter) || 0;
				if (reduceMotion) {
	...
	/* -----------------------------------------------------------------------
	 * FAQ accordion — exclusive open within a [data-dmf-accordion] group.
	 * -------------------------------------------------------------------- */
	function initAccordions() {
		document.querySelectorAll('[data-dmf-accordion]').forEach((group) => {
