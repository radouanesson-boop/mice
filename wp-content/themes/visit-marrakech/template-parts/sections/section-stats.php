 ...
 *
 * Data source order: $args['items'] (block attributes) → `vm/stats` filter
...
$args = wp_parse_args(
	$args ?? array(),
	array(
		'kicker' => __( 'Proven ground', 'vm' ),
		'title'  => __( 'Trusted at the highest level', 'vm' ),
		'items'  => array(),
		'quote'  => true,
	)
);
...
if ( empty( $mcb_items ) ) {
	$mcb_items = apply_filters(
		'vm/stats',
		array(
			array( 'value' => 'COP22', 'label' => __( 'UN Climate Change Conference', 'vm' ) ),
			array( 'value' => 'IMF · World Bank', 'label' => __( 'Annual Meetings 2023', 'vm' ) ),
			array( 'value' => 'GITEX Africa', 'label' => __( 'Largest tech event in Africa', 'vm' ) ),
			array( 'value' => 'PURE Life', 'label' => __( 'Luxury experiential travel', 'vm' ) ),
		)
	);
}
